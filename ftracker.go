...
// dist возвращает дистанцию(в километрах), которую преодолел пользователь за время тренировки.
//
// Параметры:
//
...
func ShowTrainingInfo(action int, trainingType string, duration, weight, height float64, lengthPool, countPool int) string {
	...
	switch trainingType {
	case "Бег":
		dist := distance(action)                                   // дистанция бега
		speed := meanSpeed(action, duration)                       // скорость бега
		calories := RunningSpentCalories(action, weight, duration) // потраченные калории
		return fmt.Sprintf("Тип тренировки: %s\nДлительность: %.2f ч.\nДистанция: %.2f км.\nСкорость: %.2f км/ч\nСожгли калорий: %.2f\n", trainingType, duration, dist, speed, calories)
	case "Ходьба":
		dist := distance(action)                                           // дистанция хотьбы
		speed := meanSpeed(action, duration)                               // скорость хотьбы
		calories := WalkingSpentCalories(action, duration, weight, height) // потраченные калории
		return fmt.Sprintf("Тип тренировки: %s\nДлительность: %.2f ч.\nДистанция: %.2f км.\nСкорость: %.2f км/ч\nСожгли калорий: %.2f\n", trainingType, duration, dist, speed, calories)
	case "Плавание":
		dist := float64(lengthPool) * float64(countPool) / mInKm                   // дистанция плавания
		speed := swimmingMeanSpeed(lengthPool, countPool, duration)                // скорость плавания
		calories := SwimmingSpentCalories(lengthPool, countPool, duration, weight) // потраченные калории
		return fmt.Sprintf("Тип тренировки: %s\nДлительность: %.2f ч.\nДистанция: %.2f км.\nСкорость: %.2f км/ч\nСожгли калорий: %.2f\n", trainingType, duration, dist, speed, calories)
	default:
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	speed := meanSpeed(action, duration) * kmhInMsec // переводим скорость в м/с
